...
mykpsstest <- function(data, print_output=TRUE) {
  ...
  if (print_output) {
    ...
    if(result)
      cat("KPSS: *NO* unit roots, series is trend stationary -> *FAIL* to reject H0\n")
    else
      cat("KPSS: contains unit roots, series *NOT* trend stationary -> reject H0\n")
  }
  # TRUE: FAIL to reject H0
  # FALSE: reject H0
  return(result)
...
myadftest <- function(data,print_output=TRUE,lags=30) {
  ...
  if (print_output) {
    print(adf_data)
    if(result)
      cat(sprintf("ADF: presence of unit roots over %i lags, \nindicates mean drift & random walk, \nbusiness cycles present, series is *NOT* stationary -> *FAIL* to reject H0\n", lags))
    else
      cat(sprintf("ADF: contains *NO* unit roots over %i lags, \nindicates *NO* mean drift & *NO* random walk, \nbusiness cycles *NOT* present, series is stationary -> reject H0\n", lags))
  }
...
#  TRUE: FAIL to reject H0
#  FALSE: reject H0
myboxljungarchtest <- function(data,print_output=TRUE,lags=30) {
  # Independence: Box-Ljung test
  # Box.test(type='Ljung') test: check to see if p-value ($p.value) >= 0.05 (H0)
  bl_data <- Box.test(data^2,lag=lags,type='Ljung') 
  result <- bl_data$p.value >= 0.05
  if (print_output) {
    print(bl_data)
    if(result)
      cat(sprintf("Box-Ljung ARCH(m): implies constant variance over %i lags, *NO* ARCH effects present,\n *NO* variance clustering, *NO* volatility, homoscedastic -> *FAIL* to reject H0\n", lags))
    else
      cat(sprintf("Box-Ljung ARCH(m): implies *NON*-constant variance over %i lags, ARCH effects present,\n possible variance clustering, volatility, heteroscedastic -> reject H0\n", lags))
  }
  # TRUE: FAIL to reject H0
  # FALSE: reject H0
  return(result)
